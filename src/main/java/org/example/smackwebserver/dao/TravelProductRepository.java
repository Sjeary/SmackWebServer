...
public interface TravelProductRepository extends JpaRepository<TravelProduct,Long> {
    ...
    @Query("SELECT tp FROM TravelProduct tp WHERE " +
            ...
            "(:theme IS NULL OR tp.theme = :theme) AND " +
            "(:departureLocation IS NULL OR tp.departureLocation = :departureLocation) AND " +
            "(:destination IS NULL OR tp.destination = :destination) AND " +
            "(" +
            "   tp.title LIKE %:keyword% OR " +
            "   tp.departureLocation LIKE %:keyword% OR " +
            "   tp.destination LIKE %:keyword% OR " +
            "   tp.features LIKE %:keyword% OR " +
            "   tp.theme LIKE %:keyword%" +
            ") " +
            "ORDER BY tp.id DESC")
    Page<TravelProduct> searchByConditionsAndKeyword(
